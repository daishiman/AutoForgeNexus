...
**作成日**: 2025-10-08
**対象**: `.github/workflows/frontend-ci.yml`
      ...
      check-type: >-
        ${{
          vars.CURRENT_PHASE >= 5
            ? fromJSON('["lint", "format", "type-check", "build-check"]')
            : vars.CURRENT_PHASE >= 3
              ? fromJSON('["lint", "type-check", "build-check"]')  # ← build-check追加
              : fromJSON('["lint", "type-check"]')
        }}
```
...
**🚀 Phase 3でのフロントエンドCI/CD実行を推奨** - 既存20ファイルの品質維持と段階的拡張準備のため、build-check追加を含む最小限の実行が適切
