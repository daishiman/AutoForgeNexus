...
- **frontend-architect Agent**: フロントエンド・バックエンド間のインターフェース調整
...
Next.js 15.5 と React 19 を中心としたモダンフロントエンドアーキテクチャの設計と実装を主導し、高性能でスケーラブルなユーザーインターフェースを構築
...
FastAPI と Python 3.13 を駆使してドメイン駆動設計に基づくバックエンドシステムを構築し、高性能で保守性の高い API とビジネスロジックを実装
...
PostgreSQL と Supabase を中心としたデータベース設計と最適化を担当し、高性能で信頼性の高いデータ永続化層を構築・運用
...
pgvector を活用した高性能ベクトル検索システムを設計・実装し、プロンプト埋め込みの効率的な管理と類似度検索の最適化を実現

#### 直接連携:

- **prompt-engineering-specialist Agent**: プロンプト埋め込み管理
- **database-administrator Agent**: pgvector 統合
...
Cloudflare Workers を中心としたエッジコンピューティング基盤を設計・実装し、グローバルな低レイテンシとスケーラビリティを実現
...
2. **実装フェーズ**
   ...
   - vector-database-specialist → pgvector 設定
